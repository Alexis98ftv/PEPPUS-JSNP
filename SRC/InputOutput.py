...
# Line format
PosFmt = "%05d %03d %s %9.5f %9.5f %9.3f %14.3f \
%3d %3d %3d %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f \
%7.3f %7.3f".split()
...
# Header
AmbHdr = "\
#SOD        G01        G02        G03        G04        G05        G06        G07        G08        G09        G10        G11        G12        G13        G14        G15        G16        G17        G18        G19        G20        G21        G22        G23        G24        G25        G26        G27        G28        G29        G30        G31        G32        G33        G34        G35        G36\n"
...
%10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f \
%10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f \
%10.3f %10.3f %10.3f %10.3f".split()

# File columns
AmbIdx = OrderedDict({
    "SOD": 0,
    **{f"G{i:02d}": i for i in range(1, 37)}
})

# Perf - Performance Info
# Header
PerfHdr = "\
#RCVR LONREF    LATREF    DOY SAMPLES  CONVT NSVMIN NSVMAX  HPERMS   VPERMS     HPE95      VPE95      HPEMAX    VPEMAX      PDOPMAX    HDOPMAX    VDOPMAX\n"

# Line format
PerfFmt = "%s \
%10.5f %10.5f %5d %5d %5d %3d %3d %10.3f %10.3f %10.3f \
%10.3f %10.3f %10.3f %10.3f %10.3f %10.3f".split()
...
# Header
HpeHdr = "\
#BINID   BINMIN    BINMAX   NUMSAMP  FREQ\n"

# Line format
HpeFmt = "%s %10.3f %10.3f %5d %10f".split()
...
# Line format
VpeFmt = "%s %10.3f %10.3f %5d %10f".split()
...
def generatePosFile(fpos, PosInfo):
    ...
    # Purpose: generate output file with KPVT-Pos results

    # Parameters
    # ==========
    # fpos: file descriptor
    #         Descriptor for KPVT-Pos output file
    # PosInfo: dict
    ...
    Outputs["RCVR"] = PosInfo["Rcvr"]
    Outputs["LONEST"] = PosInfo["Lon"]
    Outputs["LATEST"] = PosInfo["Lat"]
    Outputs["ALTEST"] = PosInfo["Alt"]
    Outputs["CLKEST"] = PosInfo["Clk"]
    Outputs["SOL"] = PosInfo["Sol"]
    Outputs["NSVVIS"] = PosInfo["NumSatVis"]
    Outputs["NSV"] = PosInfo["NumSat"]
...
def generateAmbFile(famb, PosInfo):

    # Purpose: generate output file with KPVT-Pos results

    # Parameters
    # ==========
    # famb: file descriptor
    #         Descriptor for KPVT-Pos output file
    # PosInfo: dict
    #         Dictionary containing Pos info for the 
    #         current epoch
    ...
    Outputs["SOD"] = PosInfo["Sod"]
    for i in range(1, 37):
        Outputs[f"G{i:02d}"] = PosInfo["Amb"][i - 1]
    
    # Write line
    for i, result in enumerate(Outputs):
        famb.write(((AmbFmt[i] + " ") % Outputs[result]))
    famb.write("\n")

# End of generateAmbFile

def generatePerfFile(fperf, PerfInfo):

    # Purpose: generate output file with PERF results
    ...
    # PerfInfo: dict
    #         Dictionary containing Perf info for the 
    #         current epoch
    ...
    Outputs["LONREF"] = PerfInfo["Lon"]
    Outputs["LATREF"] = PerfInfo["Lat"]
    Outputs["DOY"] = PerfInfo["Doy"]
    Outputs["SAMPLES"] = PerfInfo["Samples"]
    Outputs["CONVT"] = PerfInfo["Convt"]
...
def generateXpeFile(fhpe, HpeHistFinal, fvpe, VpeHistFinal):

    # Purpose: generate output file with PERF results

    # Parameters
    # ==========
    # fhpe: file descriptor
    #         Descriptor for PERF histogram output file
    # fvpe: file descriptor
    #         Descriptor for PERF histogram output file
    # HpeHistFinal: dict
    #         Dictionary containing Histogram info 
    # VpeHistFinal: dict
    #         Dictionary containing Histogram info 
    ...
    # Iterar sobre cada diccionario en HpeHistFinal
    for bin_dict in HpeHistFinal:
        # Write line
        for i, result in enumerate(bin_dict):
            fhpe.write(((HpeFmt[i] + " ") % bin_dict[result]))
        fhpe.write("\n")
    
    # Iterar sobre cada diccionario en VpeHistFinal
    for bin_dict in VpeHistFinal:
        # Write line
        for i, result in enumerate(bin_dict):
            fvpe.write(((VpeFmt[i] + " ") % bin_dict[result]))
        fvpe.write("\n")

   
# End of generateXpeFile()
